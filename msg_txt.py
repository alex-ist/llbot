...
def msg01_welcom():
    ...
    #         "Добро пожаловать на борт, и пусть путь к знанию будет надежным и интересным! 🎈")
        ...
        return  ("🎈 Привет! Я - LingoLink, бот для изучения иностранных слов!\n\n"
                 "⚡️ Я помогу выучить иностранные слова, используя научную методику интервального повторения. "
                 "Смогу стать твоим личным помощником в этом деле.\n\n"
                 "🔸 Я буду показывать слова именно в тот момент, когда тебе нужно их повторить для оптимального запоминания. "
                 "Хотя начать повторять их можно в удобное время позже. \n\n"
...
def msg03_first_set():
    return  ("Добавьте слова для изучения.\n\n"
             "🔸 У меня есть заранее составленные группы иностранных слов по некоторым темам. "
             "Для добавления группы слов в список для изучения выбирете кнопкой тему и нажмите кнопку [Начать▶️]. "
             "После этого 30+ слов добавятся в ваш список для изучения и начнется процесс изучения.\n\n"
             "🔸 Но можно добавить свои слова командой /add, или через меню.\n\n"
             "🔸 Свободно пользуйтесь любыми командами из меню в любой момент. Первое время выполнение любой команды будет сопровождаться страничкой помощи.\n\n")
...
def msg03_first_run2(w):
    return f"Теперь я открою перевод слова \"{w}\".\n\nЕсли вы верно помните его первод, нажмите кнопку [✅ Знаю]. Если вы забыли или не знаете слово, посторайтесь его запомнить и нажмите кнопку [❌ Забыл].\n\n"
...
def msg03_first_run3(result):
    ...
    if result==0:
        return f"Ничего страшного, что вы не знали слово. Во время этой тренировки я покажу его еще - все получится.\n\n{info}"
    else:
