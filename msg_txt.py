...
def msg01_welcom():
    ...
    #         "Добро пожаловать на борт, и пусть путь к знанию будет надежным и интересным! 🎈")
        ...
        return  ("🎈 Привет! Я - LingoLink, бот для изучения иностранных слов!\n\n"
                 "⚡️ Я помогу ускорить  запоминание иностранных слов, используя научную методику интервального повторения. "
                 "Смогу стать твоим личным помощником в этом деле.\n\n"
                 "🔸Я буду показывать именно те слова и в тот момент, когда тебе нужно их повторить для оптимального запоминания. "
                 "Хотя начать повторять слова можно в любое удобное время. \n\n"
                 "Добро пожаловать на борт и легкого пути к пониманию! ") #<a href=\"https://telegra.ph/Lingo-Link-06-04\">Подробнее..</a> 
...
def msg03_first_set():
    return  ("Добавьте слова для изучения.\n\n"
             "🔸Для добавления группы слов выбирете кнопкой тему и нажмите кнопку [Начать▶️]. "
             "После этого 30+ слов по этой теме добавятся в ваш список для изучения и начнется процесс изучения.\n\n"
             "🔸Можно добавить свои слова командой /add, или через меню.\n\n"
...
def msg03_first_run1(n):
    return  (f"В вашем списке для изучения {msg_word(n)}. \n\n"
             "Сейчас я покажу вам слово из этого списка, а вы постарайтесь вспомнить его перевод. После этого нажмите на кнопку [❓❓] под словом.")

def msg03_first_run2():
    return  ("Теперь я открою перевод слова. Если вы правильно запомнили слово, нажмите кнопку [✅ Знаю]. Если вы не запомнили слово, нажмите кнопку [❌ Забыл].\n\n"
