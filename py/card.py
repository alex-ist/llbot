...
class TrainingCard:
    ...
    def SetCorrect(self, correct:bool):
        ...
        if correct:
            ...
            if self.last_training_t is not None:
                last_real_interval=datetime.datetime.now()-self.last_training_t
            else:
            ...
            if self.first_answer:
                ...
                self.last_training_t=datetime.datetime.now()
                ...
                self.last_training_t=datetime.datetime.now()
                self.next_training_t=datetime.datetime.now() + i/self.u.o_param
...
class TrainingCardSet(Singleton):
    ...
    def TCardsReadyNow(self):
        db, cursor = open_db()
        tn = datetime.datetime.now()     # tn = текущее время
    ...
    def UpdateStat(self):
        ...
        self.u.UpdateLastAccess(datetime.datetime.now())
